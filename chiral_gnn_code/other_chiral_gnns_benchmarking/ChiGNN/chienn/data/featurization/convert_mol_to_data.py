...
#bond_dirs = ['NONE', 'BEGINWEDGE', 'BEGINDASH', 'ENDDOWNRIGHT', 'ENDUPRIGHT', 'EITHERDOUBLE', 'UNKNOWN']
vdw_radii = {
    'H': 1.20, 'C': 1.70, 'B': 1.92, 'N': 1.55, 'O': 1.52, 'F': 1.47, 
    'Si': 2.10, 'P': 1.80, 'S': 1.80, 'Cl': 1.75, 'Br': 1.85, 'I': 1.98
}

def convert_mol_to_data(mol: Mol) -> Data:
    """
    Transforms a rdkit mol object into a torch_geometric Data object and adds prop as an edge feature.
    Args:
        mol: rdKit mol object.
        prop: Experimental parameter to be added as an edge feature.
...
def get_node_features(atoms: List[Atom]) -> np.ndarray:
    ...
    Returns:
        Array of node features of shape (N, 48).
    """
    ...
    for node_index, node in enumerate(atoms):
        ...
        features += [get_electronegativity(node)]  # electronegativity, dim=1
        features += [int(node.IsInRing())]  # whether atom is in a ring, dim=1
        features += [get_vdw_radius(node)]  # Van der Waals radius, dim=1
        node_features[node_index, :] = features

    return np.array(node_features, dtype=np.float32)

def get_electronegativity(node: Atom) -> float:
    """
    Returns the electronegativity for a given atom.
    """
    electronegativities = {
        'H': 2.20, 'C': 2.55, 'B': 2.04, 'N': 3.04, 'O': 3.44, 'F': 3.98, 
        'Si': 1.90, 'P': 2.19, 'S': 2.58, 'Cl': 3.16, 'Br': 2.96, 'I': 2.66
    }
    return electronegativities.get(node.GetSymbol(), 0.0)

def get_vdw_radius(node: Atom) -> float:
    """
    Returns the Van der Waals radius for a given atom.
    """
    return vdw_radii.get(node.GetSymbol(), 0.0)
